...
import com.example.findrestaurant.network.onException
import com.example.findrestaurant.network.onSuccess
...
class MainViewModel @Inject constructor() : ViewModel() {
    ...
    @Inject lateinit var InfoRepository: InfoRepository
    ...
    private val _someInfoList = MutableLiveData<List<InfoDetail>>()
    val InfoList: LiveData<List<InfoDetail>>
        get() = _someInfoList

    fun requestInfo(
        //Todo 화면에서 전달 받아야 하는 공공 api 요청에 핅요한 값이 있으면 여기를 통해 전달.
    ) {
        val tempList = mutableListOf<InfoDetail>()
        InfoList.value?.let {
            tempList.addAll(it)
        }
        ...
        viewModelScope.launch {
            ...
            try {
                //Todo 공공 api 요청 객체.
                val someInfoRequest = InfoRequest(
                    //Todo 공공 api 요청에 필요한 값들을 설정 해 줘야함.
                )
                //Todo 공공 api 요청.
                val result = InfoRepository.getInfoList(someInfoRequest)
